...
class SentenceParser {
    ...
    getValueFromType(sentence) {
        ...
        if (columnName == Columns.DATE || dateMatch) {
            const parser = new DateParser(columnName, dateMatch, comparisonOperator);
            return parser.parse();
        }
        else if (columnName == Columns.ROOM_SIZE || unitUsed) {
            const parser = new SizeParser(columnName, parseFloat(numberMatch[0]), unitUsed, comparisonOperator);
            return parser.parse();
        }
        else if (columnName == Columns.CATEGORY || categoryMatch) {
            const parser = new CategoryParser(columnName, categoryMatch);
            return parser.parse();
        }  
    }
